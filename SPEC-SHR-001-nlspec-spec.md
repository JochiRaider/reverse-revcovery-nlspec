---
doc_id: SPEC-SHR-001
title: NLSpec, A grounding document for agents that read, write, implement, and evaluate natural language specifications.
status: active
version: 0.3.2
---
...
## Conformance Scope and Interpretive Boundary

This document states its conformance claims in natural language. It does not import the `MUST` / `SHOULD` / `MAY` convention defined in SPEC-SHR-000, and readers should not reinterpret its prose as if that keyword system were silently in force here. The interpretive question for this document is not which modal verb appears in a sentence. It is which sections carry conformance weight.

The conformance-bearing framework of this document lives in the following sections:

- **The Nature of the Artifact** defines the artifact class, the direction of authority, and the fact that the NLSpec is the thing the implementation must satisfy.
- **Why NLSpecs Work When They Work** defines the five structural requirements by which faithful implementation becomes possible.
- **What Completeness Means** defines completeness, the precision-completeness distinction, and the recreatability test.
- **Conceptual Fidelity** defines the representational-correctness test for whether the spec models the domain or merely its encoding.
- **Spec Economy** defines the minimize-context-without-loss discipline and the sentence-level economy test.
- **Intentional vs. Accidental Ambiguity** defines how gaps, silence, and implementer freedom are to be interpreted.

The following main-body sections are explanatory and comparative. They help the reader locate the artifact in a broader documentation ecology, but they do not add new conformance tests beyond the framework above:

- **What an NLSpec Is Not**
- **How NLSpecs Relate to Each Other**
- **Specs in Iterative Development**

The following sections are advisory guidance:

- **The Structure of NLSpecs in Practice**
- **Appendix A** through **Appendix D**

Examples that appear inside a conformance-bearing section are illustrative unless the section explicitly says that the example itself defines a required test, representation, or closure rule. Adjacency to a conformance-bearing sentence does not, by itself, make an example binding.

Within this document, Appendix D remains advisory. A recovery workflow specification that wants one of Appendix D's evaluation questions to become binding must restate that question normatively or import it explicitly at its own conformance-bearing site. `SPEC-SHR-002` is the current shared workflow specification that performs those recovery-specific imports and restatements.

Any future section not named here defaults to explanatory or advisory status. A later section does not acquire conformance force merely by existing. A later revision must classify it explicitly here, or in a successor section that explicitly replaces this one, before readers may treat it as conformance-bearing.

This section classifies interpretive weight only. It does not restate, narrow, or replace the substantive tests already defined in the conformance-bearing sections above.
...
**Definition.** Behavioral completeness means the specification determines every externally observable behavior that affects correctness or interoperability for the declared scope while leaving non-load-bearing internal mechanism open. If two competent implementers independently build from the spec without communicating, their implementations are interchangeable from the perspective of any caller.
...
**Counterexample.** A spec that says "handle errors appropriately" is behaviorally incomplete. It forces the implementer to make judgment calls about what "appropriate" means, and different implementers will make different calls, producing implementations that are mutually incompatible. That's a spec failure.

### 2. Unambiguous Interfaces

**Definition.** Unambiguous interfaces mean every boundary where components meet is specified precisely enough that independent implementations can connect without guesswork. Interfaces include function signatures, data structures, wire formats, error types, event schemas, configuration surfaces, and state contracts.

**Illustration.** A spec that defines a `Response` record with fields `id`, `model`, `provider`, `message`, `finish_reason`, `usage`, `raw`, `warnings`, `rate_limit` has made the boundary explicit. Each field has a type. Each type is defined elsewhere in the spec. The relationships between them are explicit. An implementer reading this can write the data structure in any language without guessing.
...
**Definition.** Explicit defaults and boundaries mean every configurable value has a default, every range has bounds, and every optional parameter has documented behavior when omitted.

**Illustration.** A spec that introduces a `timeout` parameter without specifying the default, the maximum, and what happens when the timeout fires has created three implementation-divergence points. A good NLSpec closes all three: "default 10 seconds, maximum 10 minutes, on timeout: SIGTERM to process group, wait 2 seconds, SIGKILL, return partial output with timeout message."
...
**Definition.** When a system interacts with multiple external systems that model the same concept differently, the spec provides explicit translation tables rather than leaving the translation to implementer intuition.
...
**Definition.** Testable acceptance criteria mean the spec includes a concrete, checkable Definition of Done that states what must be true when the implementation is correct. This is not a test plan, which defines how to test. It is a contract about what counts as completion.
...
In a multi-document system, economy applies across documents as well as within sentences. When a plan entry, slice contract, validation report, recovery snapshot, and promoted artifact all touch the same fact, one site should remain authoritative and the others should reference, apply, or narrow it rather than restating it as a second source of truth. Legitimate local working state is allowed. Duplicate authority is not.
...
The next section describes recurring structural patterns that often appear in successful NLSpecs. It names common solutions. It does not impose a required template, and failure to mirror its section order is not, by itself, a conformance defect.
...
The appendices that follow are advisory operating guidance. They support implementation, authoring, and evaluation practice, but they do not add conformance tests beyond the main body classified above.
...
## Appendix D: When Evaluating a Recovered NLSpec

When the artifact under review is a recovered NLSpec rather than a forward-authored one, the ordinary questions in Appendix C still apply. This appendix supplements those questions. It does not restate the recovery workflow or the promotion logic that produced the recovered artifact.

This appendix remains advisory within `SPEC-SHR-001`. A recovery workflow specification that wants one of these evaluation questions to become binding must restate it normatively or import it explicitly at its own conformance-bearing site. `SPEC-SHR-002` is the current shared workflow specification that does so for recovery-specific clause provenance, anchoring, accidental-contract representation, promotion-trace continuity, and final black-box cleanliness. `SPEC-SHR-004` is the companion specification that now owns repository-surface-family minima, packet minima, snapshot minima, and the other governed-artifact schema surfaces used by that workflow.

Reverse recovery is a repair path, not a replacement for the forward `Intent -> NLSpec -> Implementation` model. A useful shorthand chain is:

**Implementation -> Behavioral Description -> Provisional Contract -> Ratified NLSpec -> Future Implementation**

The recovered artifact may become indispensable, but it begins from weaker epistemic footing because intent has already been partially lost. That is why recovered artifacts need stronger visible provenance, scope anchoring, and authority review than forward-authored specs typically need.

In a multi-document recovery corpus, economy means define once at the authoritative site. A plan entry may legitimately restate working state. It should not quietly become a second definition site for a contract fact already owned by the contract, the validation report, or the promoted artifact.

**Apply the recreatability test against the declared scope.** The two-implementer test still checks precision. For a recovered artifact, also ask whether the declared scope and compatibility target are explicit enough that a competent implementer could recreate the recovered system shape without guessing. A recovered artifact can be precise about the clauses it contains and still be incomplete because the recovery effort never reached some load-bearing surface.

**Check that the recovery provenance makes each clause reviewable.** A reviewer who was not present during recovery should be able to see why each clause is in the spec, what witnesses support it, what was inferred, and what remains bounded by scope or uncertainty. If the clause reads as plausible but its basis is opaque, the artifact is not yet ready to evaluate as a trustworthy NLSpec.

**Distinguish intentional ambiguity from recovery gaps.** In a forward-authored spec, silence often means the author deliberately left room for the implementer. In a recovered spec, silence may mean the recovery effort did not recover the behavior. When the document leaves a case open, ask whether different implementations could vary there without breaking callers. If not, the silence is probably a completeness defect, not a design choice.

**Check whether authority is traceable to witnesses rather than to narrative plausibility.** A recovered clause earns authority from runtime witnesses, boundary artifacts, matched corroborants, and explicit ratification. It does not earn authority merely because it fits the story of the system. Strong prose can make a weak clause sound convincing. Evaluation should separate readability from evidentiary force.

**Check revision and deployment anchoring.** A recovered spec always describes some concrete system shape, even when the prose is generic. Ask whether the document makes clear which revision, release family, deployment shape, environment regime, or compatibility target it covers. If that anchor is missing, reviewers cannot tell whether disagreements reflect real defects, scope drift, or temporal mismatch. For the current shared workflow, the closure sites are `SPEC-SHR-002 / Workflow Overview / Entry minima before first Slice Contract`, `SPEC-SHR-002 / Recovery snapshots and incremental resumption`, and `SPEC-SHR-004 / Recovery snapshots and incremental resumption` when the question is canonical snapshot minima rather than snapshot meaning.

**Check repository-surface-family closure for repository-shaped recovery.** When the target is repository-shaped, ask whether the recovery record explicitly classified or bounded the major repository surface families through `SPEC-SHR-004 / Repository-surface-family record for repository-shaped targets` together with `SPEC-SHR-002 / Workflow Overview / Entry minima before first Slice Contract`. A recovered NLSpec should not look complete merely because build, deployment, configuration, generated-artifact, extension, data-store, or service-topology surfaces disappeared by silence.

**Check promotion-trace continuity for multi-slice handoff.** When a promoted artifact was assembled from more than one slice, ask whether each promoted clause, compatibility clause, explicit exclusion or carve-out, and explicit out-of-scope note remains reviewable through the clause-level surface required by `SPEC-SHR-002 / Authority review operating model`, `SPEC-SHR-004 / Other governed artifacts / Authority Review Packet`, and `SPEC-SHR-002 / Promoted-output completeness and representation`. Plausible final prose is not enough when clause-level provenance is opaque.

**Treat explicit bounds as a quality signal.** A good recovered artifact says where scope was bounded, where ambiguity required authority judgment, and where evidence stayed weaker than ideal. This is not weakness in the document. It is honesty about the epistemic status of the recovery. The defect is hidden uncertainty that is presented as completeness.

**Do not confuse tool choice with spec quality.** A named harness, parser-replay tool, filesystem layer, or storage mechanism can make recovery cheaper or cleaner. It is not the quality target. The quality target remains explicit behavior, explicit defaults, black-box cleanliness, reviewable provenance, and a Definition of Done that closes the declared scope.

**Check accidental contracts explicitly.** Reverse recovery often finds behaviors that callers depend on even if the original authors did not mean to elevate them. The evaluation question is not only whether the behavior is real, but also whether the recovered spec makes clear whether it is being preserved, carved out, or intentionally excluded. A backward-compatible recovered spec that silently drops externally depended-on behavior is incomplete for its declared objective.

**Check that the final prose is black-box clean.** Recovery may require structural tracing, code reading, and mechanistic reasoning. The final NLSpec should still state what must hold at the boundary, not how the current implementation happens to be decomposed, unless that decomposition is itself load-bearing. Strong provenance does not excuse implementation residue in the final contract.

**Then re-run the ordinary Appendix C questions.** Recovery-specific evaluation supplements ordinary NLSpec evaluation. It does not replace it. Once provenance, scope anchoring, and recovery gaps have been examined, reapply the usual tests for precision, boundary clarity, mapping completeness, Definition of Done alignment, intentional ambiguity, and temporal assumptions.

## Provenance and modification notice

Derived from `nlspec-spec.md` in `TG-Techie/NLSpec-Spec`, licensed under Apache-2.0.
